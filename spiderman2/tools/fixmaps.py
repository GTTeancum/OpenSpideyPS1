"""
Rebuilds the function maps for the Spider-Man port.
...
This pass rebuilds the map from three trustworthy sources instead:
  ...
  2. Pointer targets -- raw pointer words and lui/addiu pairs from any module -- that
     land on a real boundary. This is how the libgpu driver vtable and each overlay's
     own vtable are reached: an overlay's first function does nothing but store
     function pointers into the node the loader passes it, and many of those are leaf
     functions with no stack frame, which RecompOne's prologue heuristic cannot see.
...
# Highest address that still holds code in the main executable. GetTPage is the last
# PsyQ function linked in; its tail ends here and the string pool starts immediately
# after, which you can see in the raw words at 0x800917A0.
MAIN_TEXT_LIMIT = 0x8009D578
MAIN_ENTRY = 0x80093C68        # pc0 from the PS-X EXE header

# Overlays live above the 2 MB the game's own allocator can reach; see tools/overlays.py.
ADDR_LO = 0x80010000
ADDR_HI = 0x802A0000
...
class Module:
    ...
    def constants(self):
        ...
        lui = {}
        for w in self.w:
            if (w & 3) == 0 and ADDR_LO <= w < ADDR_HI:
                out.add(w)
            op = w >> 26
            rs = (w >> 21) & 31
            rt = (w >> 16) & 31
...
def main():
    ...
    # The main module comes off the disc; the overlays come from the pre-relocated
    # images, because the bytes inside CD.WAD are not the bytes that execute.
    mods = []
    exe = open(os.path.join(root, 'extracted', 'SLUS_013.78'), 'rb').read()
    mods.append(Module('main', exe[0x800:0x800 + 0xBF800], 0x80010000, MAIN_TEXT_LIMIT))

    manifest = json.load(open(os.path.join(root, 'config', 'overlays', 'manifest.json')))
    for name, v in manifest.items():
        img = open(os.path.join(root, 'config', 'overlays', name + '.bin'), 'rb').read()
        mods.append(Module(name, img, int(v['base'], 16)))
