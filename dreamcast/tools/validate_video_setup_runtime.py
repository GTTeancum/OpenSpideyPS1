...
Run wide first, then minimum: the latter restarts with the settings saved by wide.
...
def main():
    ...
    parser.add_argument('--scenario', choices=['wide', 'minimum'], required=True)
    ...
    if args.scenario == 'wide':
        ...
        parser.error('Run the wide scenario first to create persisted settings.')
    ...
    assert f'apply {expected} wide={wide}' in log
    ...
    assert log.count('[video-menu] opened') == (3 if wide else 2)
    assert log.count('[video-menu] closed; native OPTIONS restored') == (3 if wide else 2)
    assert log.count(f'opened {expected} wide={wide}') == (2 if wide else 1)
    if not wide:
    ...
    report = dict(game=args.game, scenario=args.scenario, executableSha256=hashlib.sha256(exe.read_bytes()).hexdigest(),
        selected=expected, widescreen=wide, nativeMenuOpens=3 if wide else 2,
        nativeMenuAllocationsFreed=log.count('arena free native video menu labels'),
        outputAreaVerified=True, settingsVerified=True,
