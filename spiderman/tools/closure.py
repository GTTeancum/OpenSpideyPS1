...
matters: each overlay gets its own fixed base, so a target found in shell.cs belongs
to shell and nowhere else -- but a constant call from main into overlay space cannot
say which overlay was resident, which fixmaps.py resolves by address containment.
...
def main():
    ...
    for module, hits in sorted(called.items()):
        missing = sorted(a for a in hits - defined if 0x80010000 <= a < 0x802A0000)
        if not missing:
            continue
