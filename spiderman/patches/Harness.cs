...
/// not drift.
///
///   SPIDEY_RAMDUMP=2700,2900     write ram-&lt;frame&gt;.bin at these frames
///   SPIDEY_POKE=3000:80098abc=5  write a byte (or =5:w / =5:l for 16/32-bit)
///   SPIDEY_WATCH=80098abc,4      log this address every frame it changes
///
...
public static class Harness
{
    ...
    {
        ParseDumps(Environment.GetEnvironmentVariable("SPIDEY_RAMDUMP"));
        ParsePokes(Environment.GetEnvironmentVariable("SPIDEY_POKE"));
        ParseWatches(Environment.GetEnvironmentVariable("SPIDEY_WATCH"));
        _dir = Environment.GetEnvironmentVariable("SPIDEY_RAMDIR") ?? "ram";
        if (!_active) return;
